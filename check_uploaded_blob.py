...
# Replace these with your actual Storage Account Name and Account Key
...
def get_blob_size(video_id,rendition,):
    ...
    # Assuming the video_id is used as the blob name (adjust this based on your naming convention)
    ...
    try:
        ...
        # Handle the exception if the blob doesn't exist
        return None

total_success_blob = 0
total_failed_blob = 0
...
#         # Check if the blob with the video_id exists
#         # size = get_blob_size(video_id)
#         # if size is not None:
#         #     print(f"Blob with video_id {video_id} exists. Size: {size} bytes.")
#         # else:
#         #     print(f"Blob with video_id {video_id} does not exist.")
